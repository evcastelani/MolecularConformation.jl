...
function perfomance_profile(c, method = ["Alg $(i)" for i=1:size(c,2)], color = [:black,:gray],style = [:dot,:solid])
    ...
    plot!(xaxis=:log)
    for i = 1:size(c, 2)
        plot!(t, [sum(R[:,i] .<= ti)/size(c,1) for ti in t], label=method[i], t=:steppre, lw=2,linecolor = color[i],line = style[i],legend=:bottomright)
    end
    ylims!(0, 1)
    savefig("tp_perf.pdf")
end
...
D = readdlm("results.csv",',')

perfomance_profile(D[2:end,[16,17]],["MatrixBP","QuaternionBP"])
