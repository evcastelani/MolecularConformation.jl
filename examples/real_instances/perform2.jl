...
function performRMSD(time_limit,opwrite::String="a",f::Function=median; list_of_problems="", ε=1.0e-3, virtual_ε=1.0e-8)
	# setup 
...
function performoneRMSD(prob, bpall=false, time_limit=Second(120.0); ε=1.0e-3, virtual_ε=1.0e-8)
    optc = ConformationSetup(ε, classicBP, bpall, true, virtual_ε)
    data = preprocessing("toyinstance.nmr")

    global solc = conformation(data, optc, time_limit)
